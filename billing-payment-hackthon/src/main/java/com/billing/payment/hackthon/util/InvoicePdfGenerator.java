...
public class InvoicePdfGenerator {
	...
	public static ByteArrayInputStream invoicePDFReport(List<BillingDTO> invoices) {
        ...
        try {
			...
			Stream.of("Invoice Id: " + "invoice-12345",
					/* "Patient Id: "+"xxx12345", */"Invoice Date: "+new SimpleDateFormat("dd-mm-yyy hh:mm:ss").format(new Date()))
			.forEach(t -> {
			...
			Stream.of("PATIENT ID","PATIENT NAME", "ITEM", "DESCRIPTION", "AMOUNT")
			    .forEach(headerTitle -> {
			          PdfPCell header = new PdfPCell();
			...
			Stream.of("Total Amount: "+ new DecimalFormat("0.00").format(total))
			.forEach(t -> {
				PdfPCell header = new PdfPCell();
